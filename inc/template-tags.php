...
function posted_on() {
	...
	$posted_on = sprintf(
		...
		_x( 'Posted on %s', 'post date', 'civil-cms' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		/* translators: %s: post author */
		_x( 'by %s', 'post author', 'civil-cms' ),
...
function entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( __( ', ', 'civil-cms' ) );
		if ( $categories_list ) {
			/* translators: 1: list of post categories */
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'civil-cms' ) . '</span>', wp_kses_post( $categories_list ) );
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'civil-cms' ) );
		if ( $tags_list ) {
			/* translators: 1: list of tags */
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'civil-cms' ) . '</span>', wp_kses_post( $tags_list ) );
		}
	}
		...
		comments_popup_link(
			sprintf(
				wp_kses(
					...
					__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'civil-cms' ),
					array(
						'span' => array(
							'class' => array(),
						),
	...
	edit_post_link(
		sprintf(
			wp_kses(
				...
				__( 'Edit <span class="screen-reader-text">%s</span>', 'civil-cms' ),
				array(
					'span' => array(
						'class' => array(),
					),
